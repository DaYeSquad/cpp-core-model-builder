...
std::unique_ptr<Tag> TagManager::FetchTagFromCacheByTagId(const std::string& tag_id) const {
  string where_condition = kTagId + "='" + tag_id + "'";

  LockMainDatabase();

  tags_tb_->open(where_condition);

  if (tags_tb_->recordCount() != 0) {
    sql::Record* record = tags_tb_->getRecord(0);
    unique_ptr<Tag> rtn(TagFromRecord(record));
    UnlockMainDatabase();
    return rtn;
  }
...
void TagManager::DeleteTagsFromCacheByType(ApplicationType type) const {
  ...
  tags_tb_->deleteRecords(where_condition);

  UnlockMainDatabase();
}

void TagManager::DeleteTagsFromCache() const {
...
void TagManager::DeleteTagFromCacheByTagId(const std::string& tag_id) const {
  string where_condition = kTagId + "='" + tag_id + "'";
